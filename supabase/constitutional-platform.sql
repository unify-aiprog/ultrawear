...
create table if not exists sports_source_observations (
  ...
  source_type text not null check (source_type in ('official','secondary','community','editorial')),
  entity_type text not null,
  entity_id text not null,
  observed_at timestamptz not null,
  freshness_at timestamptz,
  verification text not null check (verification in ('verified','unverified','conflicted','stale')),
...
create table if not exists content_claims (
  ...
  contradicted_by jsonb not null default '[]'::jsonb,
...
create table if not exists trend_signals (
  ...
  title text not null,
  observed_at timestamptz not null,
  location_scope text not null check (location_scope in ('near_you','country','region','global','personalized')),
  location_code text,
...
create table if not exists editorial_opportunities (
  id text primary key,
  trend_signal_ids jsonb not null default '[]'::jsonb,
  title text not null,
  reason text not null,
  status text not null check (status in ('queued','researching','review','accepted','rejected','expired')),
  confidence numeric not null default 0 check (confidence >= 0 and confidence <= 1),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
