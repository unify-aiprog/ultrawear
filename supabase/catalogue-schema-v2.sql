...
create table if not exists persons_v2 (
  ...
  image_url text,
  nationality_country_id text references countries(id) on delete set null,
  birth_date date,
  provider text,
  provider_id text,
  created_at timestamptz not null default now(),
...
create table if not exists team_memberships (
  ...
  joined_at date,
  left_at date,
  primary key (team_id, person_id, season_id, role)
);
...
create table if not exists events_v2 (
  id text primary key,
  sport_id text not null references sports(id) on delete restrict,
  competition_id text references competitions_v2(id) on delete set null,
  season_id text references seasons(id) on delete set null,
  home_team_id text references teams_v2(id) on delete set null,
  away_team_id text references teams_v2(id) on delete set null,
