...
create table if not exists competitions_v2 (
  ...
  competition_type text not null default 'league', gender text not null default 'men', age_group text not null default 'senior', level text, emblem_url text, provider text, provider_id text,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
create table if not exists seasons (
  id text primary key, sport_id text not null references sports(id) on delete restrict, competition_id text not null references competitions_v2(id) on delete cascade,
  name text not null, slug text not null, start_date date, end_date date, current boolean not null default false, provider text, provider_id text,
...
create table if not exists organizations (
  id text primary key, sport_id text not null references sports(id) on delete restrict, country_id text references countries(id) on delete set null,
  name text not null, slug text not null unique, organization_type text not null default 'club', logo_url text, provider text, provider_id text,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
create table if not exists teams_v2 (
  id text primary key, sport_id text not null references sports(id) on delete restrict, organization_id text references organizations(id) on delete set null,
  country_id text references countries(id) on delete set null, name text not null, short_name text, slug text not null unique,
  team_type text not null default 'club', gender text not null default 'men', age_group text not null default 'senior', level text, crest_url text, provider text, provider_id text,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
create table if not exists team_competitions (
  id bigserial primary key, team_id text not null references teams_v2(id) on delete cascade, competition_id text not null references competitions_v2(id) on delete cascade,
  season_id text not null references seasons(id) on delete cascade, role text not null default 'participant', created_at timestamptz not null default now(), unique (team_id, competition_id, season_id)
);
create table if not exists persons_v2 (
  id text primary key, sport_id text not null references sports(id) on delete restrict, name text not null, slug text not null unique, person_type text not null default 'player',
  image_url text, nationality_country_id text references countries(id) on delete set null, birth_date date, provider text, provider_id text,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
create table if not exists team_memberships (
  ...
  season_id text not null references seasons(id) on delete cascade, role text not null default 'player', shirt_number integer, position text, joined_at date, left_at date,
  unique (team_id, person_id, season_id, role)
);
create table if not exists venues (
  id text primary key, name text not null, slug text not null unique, country_id text references countries(id) on delete set null, city text, capacity integer, image_url text, provider text, provider_id text,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
create table if not exists events_v2 (
  id text primary key, sport_id text not null references sports(id) on delete restrict, competition_id text references competitions_v2(id) on delete set null, season_id text references seasons(id) on delete set null,
  home_team_id text references teams_v2(id) on delete set null, away_team_id text references teams_v2(id) on delete set null, venue_id text references venues(id) on delete set null,
...
create table if not exists competition_standings (
  id bigserial primary key, competition_id text not null references competitions_v2(id) on delete cascade, season_id text not null references seasons(id) on delete cascade,
  position integer not null, team_id text not null references teams_v2(id) on delete cascade, played integer, won integer, drawn integer, lost integer, goals_for integer, goals_against integer, goal_difference integer, points integer, provider text, provider_id text,
...
create index if not exists team_competitions_team_idx on team_competitions(team_id, season_id);
...
create index if not exists events_v2_home_starts_idx on events_v2(home_team_id, starts_at desc);
create index if not exists events_v2_away_starts_idx on events_v2(away_team_id, starts_at desc);
create index if not exists events_v2_updated_idx on events_v2(updated_at desc);
