...
- `FOOTBALL_DATA_INGEST_SECRET` — long random secret for legacy/standalone ingestion endpoints.
...
Set these repository secrets for the enabled scheduled workflows:
...
5. Run catalogue ingestion manually once.
6. Run standings ingestion manually once.
7. Run live ingestion manually once.
8. Run the sports revalidation workflow once.
9. Check `/api/health` returns HTTP 200 with `status: healthy`.
...
- Verify ingestion endpoints return 401 without their secrets.
...
Do not call the site production-ready until CI is green, production environment variables are configured, both Supabase schemas are migrated, the first catalogue/standings/live/revalidation cycles succeed, the production validation workflow passes, and `/api/health` returns 200.
