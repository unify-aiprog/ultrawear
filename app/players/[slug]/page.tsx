...
export default async function PlayerPage({ params }: Props) {
  ...
  return <div className="page-wrap"><section className="page-hero"><p className="eyebrow">PLAYER PROFILE</p><h1>{person.name}</h1><p>{person.person_type} · {person.countries?.name ?? 'Nationality not listed'}</p></section>
    <section className="profile-grid"><div className="profile-card"><span>ROLE</span><strong>{person.person_type}</strong></div><div className="profile-card"><span>NATIONALITY</span><strong>{person.countries?.name ?? '—'}</strong></div><div className="profile-card"><span>BORN</span><strong>{person.birth_date ? new Date(person.birth_date).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' }) : '—'}</strong></div></section>
