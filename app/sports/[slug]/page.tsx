...
export default async function SportPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <section className="section">
      ...
      <ProgrammeSection label="NOW" title="LIVE." events={programme.now} /><ProgrammeSection label="NEXT" title="COMING UP." events={programme.next} /><ProgrammeSection label="TONIGHT" title="TONIGHT." events={programme.tonight} /><ProgrammeSection label="TOMORROW" title="TOMORROW." events={programme.tomorrow} /><ProgrammeSection label="THIS WEEKEND" title="THE WEEKEND." events={programme.thisWeekend} /><ProgrammeSection label="RECENT" title="RELIVE IT." events={programme.recent} />
...
function LeadEvent({ event }: { event: ProgrammeEvent }) { return <section className="detail-section"><div className="section-heading"><span>{event.priority}</span><h2>{event.priority === 'BLOCKBUSTER' ? 'THE BIG ONE.' : event.priority === 'LIVE' ? 'LIVE NOW.' : 'WHAT’S NEXT.'}</h2></div><div className="live-card"><div className="live-card__meta"><span className="live-badge">{event.priority}</span><span>{event.competition}</span></div><div className="live-card__teams"><div><span>{event.home?.name ?? event.competition}</span><strong>{formatScore(event.homeScore)}</strong></div><div><span>{event.away?.name ?? event.stage ?? 'Programme'}</span><strong>{event.away ? formatScore(event.awayScore) : formatCountdown(event.minutesUntilStart)}</strong></div></div></div></section>; }
function ProgrammeSection({ label, title, events }: { label: string; title: string; events: ProgrammeEvent[] }) { if (!events.length) return null; return <section className="detail-section"><div className="section-heading"><span>{label}</span><h2>{title}</h2></div><div className="index-grid">{events.map((event) => <div className="index-card" key={event.id}><span>{event.priority} · {event.competition}</span><b>{event.home?.name ?? event.competition}{event.away ? ` v ${event.away.name}` : ''}{event.status === 'IN_PLAY' || event.status === 'PAUSED' ? ` · ${formatScore(event.homeScore)} — ${formatScore(event.awayScore)}` : ''}</b><small>{event.status === 'IN_PLAY' || event.status === 'PAUSED' ? `Live · ${event.provider}` : event.minutesUntilStart !== undefined ? `Starts in ${Math.max(0, event.minutesUntilStart)} minutes.` : event.status === 'FINISHED' ? `Final · ${event.provider}` : 'Start time TBC.'}</small></div>)}</div></section>; }
function formatScore(score: number | null | undefined) { return score === null || score === undefined ? '—' : String(score); }
function formatCountdown(minutes: number | undefined) { return minutes === undefined ? 'TBC' : `IN ${Math.max(0, minutes)}M`; }
