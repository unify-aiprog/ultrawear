...
export async function GET() {
  ...
  if (stored?.programme && !isProgrammeStale(stored.updatedAt)) return NextResponse.json({ generatedAt: stored.updatedAt, mode: 'sports-brain', stale: false, programme: stored.programme }, { headers: { 'Cache-Control': 'no-store' } });
  if (stored?.programme) return NextResponse.json({ generatedAt: stored.updatedAt, mode: 'sports-brain-stale', stale: true, programme: stored.programme }, { headers: { 'Cache-Control': 'no-store' } });

  const experiences = liveExperiences();
  const programme = buildSportsProgramme(experiences.map((experience) => ({
