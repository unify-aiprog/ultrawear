...
function rowToObservation(row: { id: string; source_id: string; source_type: SourceObservation['sourceType']; observed_at: string; freshness_at: string | null; verification: SourceObservation['verification']; confidence: number | string; payload: unknown }): SourceObservation {
  return createObservation({ id: row.id, sourceId: row.source_id, sourceType: row.source_type, observedAt: row.observed_at, freshnessAt: row.freshness_at ?? undefined, verification: row.verification, confidence: Number(row.confidence), payload: row.payload });
}
export const footballMatchAdapter: SportsProviderAdapter<FootballDataMatch> = {
  ...
  normalize: (match) => ({ entityType: 'event', entityId: `event_footballdataorg_${match.id}`, observedAt: new Date().toISOString(), freshnessAt: match.status === 'FINISHED' ? undefined : new Date(Date.now() + 5 * 60_000).toISOString(), payload: { sport: 'football', providerId: String(match.id), status: match.status, startTime: match.utcDate, competitionId: String(match.competition.id), seasonId: String(match.season.id), homeTeamId: String(match.homeTeam.id), awayTeamId: String(match.awayTeam.id), homeScore: match.score?.fullTime?.home ?? null, awayScore: match.score?.fullTime?.away ?? null } }),
};
