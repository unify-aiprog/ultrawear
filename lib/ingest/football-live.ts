...
export async function ingestFootballLive() {
  ...
  for (const match of matches) {
    await upsertLiveMatch(supabase, match);
    updated++;
  }

  // A completed match must stop appearing as LIVE. The normal catalogue sync
  // will persist its final score; this query keeps the live surface clean.
  return { live: matches.length, updated };
}
...
async function upsertLiveMatch(supabase: NonNullable<ReturnType<typeof getSupabaseServerClient>>, match: FootballDataMatch) {
  ...
  await supabase.from('events_v2').upsert({
    ...
    status: LIVE_STATUSES.has(match.status) ? match.status : match.status,
    home_score: match.score?.fullTime?.home ?? null,
    away_score: match.score?.fullTime?.away ?? null,
    round_name: match.stage ?? match.group ?? (match.matchday ? `Matchday ${match.matchday}` : null),
    provider: PROVIDER,
...
async function ensureTeam(supabase: NonNullable<ReturnType<typeof getSupabaseServerClient>>, providerId: number, name: string, crest?: string | null) {
  ...
  const { data } = await supabase.from('teams_v2').upsert({
    id, sport_id: 'football', name, slug: slugify(`${name}-${providerId}`), team_type: 'CLUB', gender: 'MEN', age_group: 'SENIOR', crest_url: crest ?? null, provider: PROVIDER, provider_id: String(providerId)
  }, { onConflict: 'id' }).select('id').single();
  return data?.id ?? null;
}

function dedupe(matches: FootballDataMatch[]) {
