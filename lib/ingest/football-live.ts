...
export async function ingestFootballLive() {
  ...
  // Never replace a good live cache with an empty snapshot caused by a transient DB read failure.
...
async function ensureTeam(supabase: NonNullable<ReturnType<typeof getSupabaseServerClient>>, providerId: number, name: string, crest?: string | null) {
  ...
  const { data, error } = await supabase.from('teams_v2').upsert({ id, sport_id: 'football', name, slug: slugify(`${name}-${providerId}`), team_type: 'CLUB', gender: 'MEN', age_group: 'SENIOR', crest_url: crest ?? null, provider: PROVIDER, provider_id: String(providerId) }, { onConflict: 'id' }).select('id').single();
