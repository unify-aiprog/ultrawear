...
export async function upsertCanonicalEvent(provider: string, payload: CanonicalEventPayload) {
  ...
  const homeTeamId = `team_${providerKey(provider, homeProviderId)}`;
  const awayTeamId = `team_${providerKey(provider, awayProviderId)}`;

  const { error: competitionError } = await supabase.from('competitions_v2').upsert({
    id: competitionId, sport_id: sportId, name: `Competition ${competitionProviderId}`, slug: slugify(`${provider}-${competitionProviderId}`),
  ...
  const { error: seasonError } = await supabase.from('seasons').upsert({
    id: seasonId, sport_id: sportId, competition_id: competitionId, name: `Season ${seasonProviderId}`,
    slug: slugify(`${provider}-${seasonProviderId}`), provider, provider_id: seasonProviderId,
  }, { onConflict: 'id' });
  if (seasonError) throw new Error(`Unable to persist season: ${seasonError.message}`);

  for (const [teamId, teamProviderId] of [[homeTeamId, homeProviderId], [awayTeamId, awayProviderId]] as const) {
    const { error: teamError } = await supabase.from('teams_v2').upsert({
      id: teamId, sport_id: sportId, name: `Team ${teamProviderId}`, slug: slugify(`${provider}-${teamProviderId}`), provider, provider_id: teamProviderId,
    }, { onConflict: 'id' });
  ...
  const { error: eventError } = await supabase.from('events_v2').upsert({
    ...
    provider, provider_id: payload.providerId,
  }, { onConflict: 'id' });
