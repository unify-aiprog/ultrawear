...
export async function saveServerSportsIdentity(identity: SportsIdentity) {
  ...
  if (supabase) {
    const { error } = await supabase.from('sports_identities').upsert({
      ...
      followed_teams: next.followedTeams,
      followed_athletes: next.followedAthletes,
      followed_communities: next.followedCommunities,
      xp: next.xp,
      level: next.level,
      badges: next.badges,
...
function rowToIdentity(row: Record<string, unknown>): SportsIdentity {
  return {
    ...
    followedTeams: (row.followed_teams as string[]) ?? [],
    followedAthletes: (row.followed_athletes as string[]) ?? [],
    followedCommunities: (row.followed_communities as string[]) ?? [],
    xp: Number(row.xp ?? 0),
