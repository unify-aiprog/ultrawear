...
export type FootballDataCompetition = {
  id: number; name: string; code: string | null; type: string; plan?: string | null; emblem?: string | null;
  area: FootballDataArea; currentSeason?: FootballDataSeason | null; seasons?: FootballDataSeason[];
};
export type FootballDataPerson = {
  id: number; name: string; firstName?: string | null; lastName?: string | null; dateOfBirth?: string | null;
  nationality?: string | null; position?: string | null; role?: string | null;
};
export type FootballDataTeam = {
  id: number; name: string; shortName?: string | null; tla?: string | null; crest?: string | null;
  area?: FootballDataArea | null; venue?: string | null; founded?: number | null; website?: string | null;
  squad?: FootballDataPerson[];
};
export type FootballDataMatch = {
  id: number; utcDate: string; status: string; matchday?: number | null; stage?: string | null; group?: string | null;
  venue?: string | null; competition: { id: number; name: string; code?: string | null }; season: FootballDataSeason;
...
async function request<T>(path: string): Promise<T> {
  ...
  const response = await fetch(`${API_BASE}${path}`, {
    headers: { Accept: 'application/json', 'X-Auth-Token': token }, cache: 'no-store'
  });
  if (!response.ok) {
...
export async function listCompetitionMatches(codeOrId: string | number, params: Record<string, string | number | undefined> = {}) {
  ...
  return request<{ count: number; competition: FootballDataCompetition; season: FootballDataSeason; matches: FootballDataMatch[] }>(`/competitions/${encodeURIComponent(String(codeOrId))}/matches${query.size ? `?${query}` : ''}`);
}
export async function listMatches(params: Record<string, string | number | undefined> = {}) {
  const query = new URLSearchParams();
  for (const [key, value] of Object.entries(params)) if (value !== undefined) query.set(key, String(value));
  return request<{ count: number; matches: FootballDataMatch[] }>(`/matches${query.size ? `?${query}` : ''}`);
}
