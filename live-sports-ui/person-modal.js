...
function normalizePerformances(person) {
  return Array.isArray(person?.recentPerformances) ? person.recentPerformances.slice(0, 10) : [];
}
...
function renderPerformance(item) {
  const competition = item.competition?.name || item.competition || 'Event';
  const team = item.teamId || '';
  const opponent = item.opponentId || '';
  const stats = item.stats && typeof item.stats === 'object' ? Object.entries(item.stats).slice(0, 3).map(([key, value]) => `${key}: ${value}`).join(' · ') : '';
  return `<article class="person-modal-performance"><div><b>${esc(competition)}</b><span>${esc(formatDate(item.startsAt))}</span></div><p>${esc([team, opponent].filter(Boolean).join(' vs '))}</p>${stats ? `<small>${esc(stats)}</small>` : ''}</article>`;
}
...
function render(person, entityType) {
  ...
  const performances = normalizePerformances(person);
  ...
  backdrop.innerHTML = `<section class="person-modal" role="dialog" aria-modal="true" aria-labelledby="person-modal-title"><button class="person-modal-close" type="button" aria-label="Close profile">×</button><div class="person-modal-head"><div class="person-modal-avatar">${person.image ? `<img src="${esc(person.image)}" alt="">` : esc(firstName(person.name))}</div><div><p class="eyebrow">${esc(roles.join(' · '))}</p><h2 id="person-modal-title">${esc(person.name || 'Sports person')}</h2>${person.currentTeamId ? `<p>Current team: ${esc(person.currentTeamId)}</p>` : ''}</div></div><div class="person-modal-meta">${meta.map((item) => `<span class="person-modal-chip">${esc(item)}</span>`).join('')}</div>${stats.length ? `<div class="person-modal-section"><h3>Stats</h3><div class="person-modal-stats">${stats.map(([label,value]) => `<div class="person-modal-stat"><b>${esc(value)}</b><span>${esc(label)}</span></div>`).join('')}</div></div>` : ''}${performances.length ? `<div class="person-modal-section"><h3>Recent performances</h3><div class="person-modal-performances">${performances.map(renderPerformance).join('')}</div></div>` : `<div class="person-modal-section"><h3>Performance history</h3><p class="person-modal-note">Verified performance statistics will appear here as canonical event data becomes available.</p></div>`}<div class="person-modal-section"><h3>Career</h3><p class="person-modal-note">This compact profile uses the shared person identity. Player, athlete and manager histories can be connected without creating another page for every role.</p></div></section>`;
  ...
  const onKey = (event) => { if (event.key === 'Escape') { closeModal(); document.removeEventListener('keydown', onKey); } };
