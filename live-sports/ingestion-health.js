...
export function createIngestionHealth({ policy, now } = {}) {
  ...
  return Object.freeze({
    record(result) {
      return tracker.record({ sourceId: result?.sourceId, eventStatus: result?.event?.status ?? null, observedAt: result?.observedAt ?? null, ok: result?.ok === true, latencyMs: result?.latencyMs ?? null, message: result?.error ?? null });
    },
    get: tracker.get,
    list: tracker.list,
  });
