...
import { createFactAuditRecord } from './fact-audit.js';

const RECONCILED_FIELDS = Object.freeze(['sport', 'competition', 'startsAt', 'status', 'score', 'venue']);
...
export function chooseCanonicalEvent({ existing = null, incoming, reconciledFields }) {
  ...
  for (const item of reconciledFields) {
    if (item.value != null && item.verification !== 'conflicted') canonical = eventWithField(canonical, item.field, item.value);
  }
...
export function createEventReconciler({ eventStore, observationStore, auditStore = null, sourceConfidence = () => 0.7 }) {
  if (!eventStore || typeof eventStore.getEvent !== 'function' || typeof eventStore.putEvent !== 'function' || typeof eventStore.listEvents !== 'function') throw new TypeError('Event store with getEvent, putEvent and listEvents is required');
  if (!observationStore || typeof observationStore.putObservation !== 'function' || typeof observationStore.listObservations !== 'function') throw new TypeError('Observation store is required');
  if (auditStore && typeof auditStore.putAudit !== 'function') throw new TypeError('Audit store must provide putAudit');
  ...
  return Object.freeze({
    async reconcile({ sourceId, event: incoming, observedAt, sourceType = 'open' }) {
      ...
      for (const field of RECONCILED_FIELDS) {
        ...
        if (!fieldObservations.length) continue;
      ...
      return {
        event: canonical, canonicalId, matched: Boolean(existing), matchScore: match?.score ?? 0,
        observations, reconciledFields, audits,
