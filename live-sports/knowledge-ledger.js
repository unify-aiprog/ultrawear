...
export function reconcileObservations(observations = []) {
  ...
  return [...groups.values()].map((group) => {
    ...
    const ranked = [...byValue.values()].sort((a, b) => {
      const score = (items) => items.reduce((sum, item) => sum + item.confidence, 0);
      return score(b) - score(a);
    });
    const winner = ranked[0] ?? [];
    const state = ranked.length > 1 ? 'conflicted' : winner.length > 1 ? 'corroborated' : 'unverified';
    return {
      ...
      verification: state,
      sources: group.map((item) => item.sourceId),
